...
void borrow_books(Person* person, Library* library) {
    ...
    for (i = 0; i < num_books; i++) {
        ...
        if (!found) {
            for (j = 0; j < library->num_donated_books; j++) {
                if (strcmp(person->loans[person->num_loans + i].title, library->donated_books[j].title) == 0 &&
                    strcmp(person->loans[person->num_loans + i].author, library->donated_books[j].author) == 0) {
                    found = 1;
                    break;
                }
            }
...
void search_books(Library* library) {
    ...
    for (i = 0; i < library->num_donated_books; i++) {
        if (strstr(library->donated_books[i].title, title) || strstr(library->donated_books[i].author, title)) {
            printf("%s by %s - Donated\n", library->donated_books[i].title,
                library->donated_books[i].author);
            found = 1;
        }
    }
...
// Donate books function
void donate_books(Library* library) {
    if (library->num_donated_books >= MAX_NUM_BOOKS) {
        printf("Sorry, the library cannot accept more donated books at the moment.\n");
        return;
    }

    printf("Enter the title and author of the book to donate:\n");
    printf("Title: ");
    scanf_s("%s", library->donated_books[library->num_donated_books].title,
        sizeof(library->donated_books[library->num_donated_books].title));
    printf("Author: ");
    scanf_s("%s", library->donated_books[library->num_donated_books].author,
        sizeof(library->donated_books[library->num_donated_books].author));

    library->num_donated_books++;
